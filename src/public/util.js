...
util.getCurrencySymbol = (coin = store.state.user.coin) => {
  return store.state.user.coinUnit[coin];
}
...
util.sendAppMsg = (params) => {
  if(util.browser.versions.android&&AZEX){
    AZEX.postMessage(JSON.stringify(params));
    return;
  }
  if(util.browser.versions.iPhone && window.webkit){
    window.webkit.messageHandlers.AZEX.postMessage(JSON.stringify(params));
    return;
  }
  if(!util.browser.versions.mobile){
    let url;
    switch(params.func){
      case('login'): url = '/login/login';break;
      case('bound_phone'): url = '/personal/main/accountSettingHome';break;
      case('SetTradePassword'): url = '/personal/accountSetting/setTradePwd';break;
      default: url ='';
    }
    if(url){
      window.open(window.location.origin+url+'?backUrl='+window.location.href)
    }
  }
}
